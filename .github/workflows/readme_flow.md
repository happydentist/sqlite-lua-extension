...
graph TD
    %% 觸發機制
    Start([⚙️ 網頁手動觸發 workflow_dispatch]) --> CheckOut[📋 actions/checkout 拉取專案原始碼]
    
    %% 多平台並行
    CheckOut --> JobWin[🪟 Windows 環境 windows-latest]
    CheckOut --> JobLin[🐧 Linux 環境 ubuntu-latest]
    
    %% Windows 分支
    subgraph Windows 建構流程
        JobWin --> MSYS2[🔧 啟動 MSYS2 環境 MINGW64]
        MSYS2 --> CheckLuaWin[📥 Checkout Lua / LuaJIT 原始碼]
        CheckLuaWin --> PolyfillWin[💉 注入 Polyfill 巨集補丁]
        PolyfillWin --> BuildStaticWin[📦 編譯 liblua.a / libluajit.a 靜態庫]
        BuildStaticWin --> GccWin[🔨 GCC 強制連結靜態庫 & 導出符號]
        GccWin --> ArtifactWin([📦 輸出獨立靜態成品的 sqlite-lua.dll])
    end
    
    %% Linux 分支
    subgraph Linux 建構流程
        JobLin --> AptSql[🔧 Apt 安裝 libsqlite3-dev]
        AptSql --> CheckLuaLin[📥 Checkout Lua / LuaJIT 原始碼]
        CheckLuaLin --> PolyfillLin[💉 注入 Polyfill 巨集補丁]
        PolyfillLin --> BuildStaticLin[📦 編譯帶有 -fPIC 的 liblua.a 靜態庫]
        BuildStaticLin --> GccLin[🔨 GCC 強制連結靜態庫 & -shared -fPIC]
        GccLin --> ArtifactLin([📦 輸出獨立靜態成品的 sqlite-lua.so])
    end
```
